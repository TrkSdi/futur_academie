...
for index, program in enumerate(list_cod_aff_form):
    ...
    try:
        ...
        span_elements = []
    try:
        div_elements = infos.find_all(
            "div", {"class": "psup-carte-texte-cle fr-mb-1w fr-mr-1w"}
        )
    except:
        div_elements = []
    try:
    ...
    for span in span_elements_encar:
        ...
        if result.isdigit() or (
            result.endswith("%") and result[:-1].isdigit()
        ):  # test si c'est un nombre ou si cela finit par un %
            if result.endswith("%"):
                result = result.strip(
                    "%"
                )  # on enlève le pourcentage pour traitement plus facile à posteriori
            dico.append(result)
    ...
    for div in div_elements:
        ...
        if "Taux de réussite en 3 ou 4 ans" in div_result:
            taux_value = (
                div_result.split("Taux de réussite en 3 ou 4 ans")[1]
                .strip()
                .split(" ")[0]
            )
        ...
        # idem, c'est soit l'un ou l'autre
            ...
            taux_value = (
                div_result.split("Taux de réussite en 2 ou 3 ans")[1]
                .strip()
                .split(" ")[0]
            )
            if taux_value.endswith("%"):
